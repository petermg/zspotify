...
def download_playlist(playlist):
    ...
    for song in p_bar:
        ...
        download_track(song[TRACK][ID], sanitize_data(playlist[NAME].strip()),disable_progressbar=True)
...
def download_from_user_playlist():
    ...
    if len(playlist_choices) == 1:
        for dic in dics:
            if str(playlist_choices[0]) == str(dic['INDEX']):
        ...
        for playlist_number in range(start, end):
            for dic in dics:
                if str(playlist_number) == str(dic['INDEX']):
                    download_playlist(dic)
