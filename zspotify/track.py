...
def get_song_info(song_id) -> Tuple[List[str], str, str, Any, Any, Any, Any, Any, Any]:
    ...
    try:
        ...
        print('###   EXCEPTION get_song_info  ###',e)
...
def download_track(track_id: str, extra_paths='', prefix=False, prefix_value='', disable_progressbar=False) -> None:
    ...
    try:
        ...
        if prefix:
            song_name = f' {artists[0]} - {album_name} - {str(track_number).zfill(2)} - {name}' if prefix_value.isdigit(
            ) else f'{track_number} - {name}'

        filename = os.path.join(download_directory, f'{song_name}.{ZSpotify.get_config(DOWNLOAD_FORMAT)}')
